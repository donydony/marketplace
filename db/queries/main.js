...
let featuredData = function (pageNumber) {
  ...
  return db
    .query(`SELECT users.id AS user_id, items.id as item_id, sold_status, img_url, price, title, items.description AS item_description, users.user_pic, users.username
    FROM items
...
let filterData = function (pageNumber, name, min, max, boolean) {
  ...
  let realMin = min * 100;
  let realMax = max * 100;
  let queryString = `SELECT users.id AS user_id, items.id as item_id, sold_status, img_url, price, title, items.description AS item_description, users.user_pic, users.username
  FROM items
  JOIN users ON items.seller_id = users.id
  `;
  let parameters = [];
